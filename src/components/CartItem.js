  ...
  return ( <>
    <li className="CartList"> 
      ...
      <div className="Name-Qty">
        ...
        <div className="Qty">
          <button onClick={handleDeleteOne} id="btn_delete">-</button>
          <p className="ItemQty">{item.qty} </p>
          <button onClick={handleAddOne} id="btn_add">+</button>
        </div>
      </div>

      <p className="Price">${item.price}</p>
      
      <p className="SubTotalPrice" >${parseInt(item.price, 10)*parseInt(item.qty, 10)}</p>
